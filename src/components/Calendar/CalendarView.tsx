...
export default function CalendarView() {
  ...
  const [calendarData, setCalendarData] = useState<ReturnType<typeof loadCalendarData> | null>(null);
  ...
  const displayMonths = viewMode === 'all' 
    ? allMonthsData 
    : allMonthsData.filter(m => m.month.toLowerCase() === viewMode.toLowerCase());
  ...
  const filteredMonths = searchQuery
    ? displayMonths.filter(month => {
        ...
        return (
          ...
          month.themes.some(theme => theme.toLowerCase().includes(searchLower)) ||
          month.events.some(e => e.event.toLowerCase().includes(searchLower)) ||
          month.highlightedDates.some(e => e.event.toLowerCase().includes(searchLower))
        );
      })
    : displayMonths;
      ...
      <div className="mb-6 bg-white rounded-xl shadow-md p-2">
        <div className="flex flex-wrap gap-2">
          ...
          {MONTH_NAMES.map(month => {
            const hasData = months.some(m => m.toLowerCase() === month.toLowerCase());
